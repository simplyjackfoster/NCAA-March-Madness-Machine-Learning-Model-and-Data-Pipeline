...
def test_matchup_matrix_probabilities_not_extreme(tmp_path, monkeypatch):
    """After calibration, no matchup probability should be > 0.95."""
    import pickle
    from sklearn.linear_model import LogisticRegression
    from src.simulation import matchup_matrix as mm_module
    from src.simulation.matchup_matrix import build_matchup_matrix
    ...
    pd.DataFrame({
        ...
        "adj_o": [120.0, 100.0, 85.0, 70.0],
        "adj_d": [70.0, 90.0, 105.0, 120.0],
        "tempo": [70.0, 68.0, 66.0, 64.0],
        "net_rating": [50.0, 10.0, -20.0, -50.0],
        "elo_pre": [1900.0, 1580.0, 1340.0, 1100.0],
        "adj_em": [30.0, 10.0, -5.0, -20.0],
        "luck": [0.0] * 4,
        "seed": [1, 8, 5, 4],
    }).to_parquet(feat_dir / f"team_season_{year}.parquet", index=False)

    # Write a prior model trained on 4 features
    FEATURES = ["elo_diff", "net_rating_diff", "tempo_diff", "seed_diff"]
    rng = np.random.default_rng(42)
    n = 200
    elo = rng.normal(0, 10, n)
    X = pd.DataFrame({
        "elo_diff": elo,
    ...
    y = (elo + rng.normal(0, 3, n) > 0).astype(int)
    ...
    # Write a calibrator that moderates extreme probabilities
    raw_probs = prior.predict_proba(X)[:, 1].reshape(-1, 1)
    platt = LogisticRegression(max_iter=1000).fit(raw_probs, y)
    cal_dir = tmp_path / "artifacts" / "calibrators"
    cal_dir.mkdir(parents=True)
    with open(cal_dir / "isotonic.pkl", "wb") as f:
        pickle.dump(platt, f)

    out = build_matchup_matrix(year, "dummy_config.yaml")
    mat = np.load(out)
    off_diag = mat[mat != 0]
    assert off_diag.max() < 0.95, f"Max prob {off_diag.max():.3f} still too extreme"
